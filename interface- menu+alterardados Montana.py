...
def adicionar_paciente():
    ...
    layout_add = [
        ...
        [sg.Text('Prioridade:', background_color=claro, text_color=escuro), 
         sg.Combo(['Baixa','Normal', 'Alta', 'Urgente'], key='-PRIORIDADE-', readonly=True)],
...
def listar_medicos():
    ...
    while True:
        ...
        if event == '-EDITAR-MED-' and medico_selecionado:
            # Separar tratamento do nome
            nome_completo = medico_selecionado['nome']
            tratamento = 'Dr.'
            nome_sem_tratamento = nome_completo
            
            # Verificar se começa com Dr. ou Dra.
            if nome_completo.startswith('Dr. '):
                tratamento = 'Dr.'
                nome_sem_tratamento = nome_completo[4:]  # Remove "Dr. "
            elif nome_completo.startswith('Dra. '):
                tratamento = 'Dra.'
                nome_sem_tratamento = nome_completo[5:]  # Remove "Dra. "
            ...
            while True:
                ...
                if event_edit == '-SALVAR-':
                    if not values_edit['-NOME-'].strip() or not values_edit['-ESPECIALIDADE-'].strip():
                        ...
                        # Concatenar tratamento + nome novamente
